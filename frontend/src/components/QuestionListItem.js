...
const QuestionListItem = (props) => {
    const { description, choices, correctAnswer } = props.question.questions.filter(question => question._id === props.id)[0];
    ...
    let editChoicesList = editChoices.map((editChoice, curIndex) => (
        <ListGroupItem key={curIndex}>
            <Button
                ...
                size='sm'
                onClick={() => {
                    setEditChoices(editChoices.filter((choice, index) => index !== curIndex ))
                }}
            >&times;</Button>
            {editChoice}
        </ListGroupItem>
    ...
    const editModal = (
        <Modal isOpen={editModalVis} toggle={toggleEditQuestion}>
            <ModalHeader toggle={toggleEditQuestion}>Create a New Question</ModalHeader>
            <ModalBody>
                <Form onSubmit={onSubmitQuestion}>
                    ...
                    <FormGroup>
                        ...
                        <Button className="mt-2" onClick={() => setEditChoices([...editChoices, curChoice])}>Add Choice</Button>
                    </FormGroup>
                    <FormGroup>
                    <Label for="desc">Correct Answer:</Label>
                        <Input type="text" name="answer" id="answer" value={editCorrectAnswer} onChange={e => setEditCorrectAnswer(e.target.value)}/>
                    </FormGroup>
                </Form>
            </ModalBody>
            <ModalFooter>
    ...
    return(
        <div>
            <Card className="question" body >
                ...
                <ButtonGroup>
                    <Button size="sm" color="primary" onClick={toggleEditQuestion}>Edit</Button>
