...
const QuestionList = props => {
  ...
  const editTitleModal = (
    <Modal isOpen={titleModalVis} toggle={toggleEditTitle}>
      ...
      <ModalFooter>
        <Button color="primary" onClick={onSubmitEditTitle}>
          Submit
        </Button>
        <Button color="secondary" onClick={toggleEditTitle}>
          Cancel
        </Button>
      </ModalFooter>
    </Modal>
  ...
  const addModal = (
    <Modal isOpen={addModalVis} toggle={toggleAddQuestion}>
        ...
        <Form onSubmit={onSubmitQuestion}>
          ...
          <FormGroup>
            ...
            <Input
              ...
              value={curChoice}
              onChange={e => {
                setCurChoice(e.target.value);
              }}
            />
            <Button
              className="mt-2"
              onClick={() => setChoices([...choices, curChoice])}
            >
              Add Choice
            </Button>
          </FormGroup>
      ...
      <ModalFooter>
        ...
        <Button color="secondary" onClick={toggleAddQuestion}>
          Cancel
        </Button>
      </ModalFooter>
    </Modal>
  ...
  if (!loading) {
    listItems = questions.map(question => (
      <QuestionListItem key={question._id} id={question._id} />
    ));
  }
